...
{
	std::wstring newText = L"Bookmark Manager: версия " + std::wstring(BOOKMARKMANAGER_VERSIONNAME);
...
INT_PTR aboutProgWnd_openLink(HWND hDlg, LPARAM lParam)
{
	...
	if ((int)errCode <= 32) {
		std::wstring errMessage = L"Сould not open link: " + std::wstring(litem.szUrl) +
